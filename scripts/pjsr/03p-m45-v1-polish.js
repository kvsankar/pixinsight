// M45/Pleiades v1 nonlinear polish.
// Starts from a MaskedStretch image. Applies restrained sky chroma cleanup,
// reflection-dust color shaping, mild local contrast, highlight restraint,
// and optional TIFF/JPEG exports.
// Usage:
//   -r=03p-m45-v1-polish.js,input=<xisf>,output=<xisf>,tiff=<tif>,jpg=<jpg>,log=<path>
...
var logPath = arg( "log", "work/logs/03p-m45-v1-polish.log" );
var f = new File;
...
try
{
   ...
   runPixelMath(
      view,
      "max(min($T[0]-" + bg + "*" + jsNum( numArg( "skyRedNeutral", 0.18 ) ) +
         "*max($T[0]-($T[1]+$T[2])/2,0),1),0)",
      "max(min($T[1]-" + bg + "*" + jsNum( numArg( "skyGreenNeutral", 0.25 ) ) +
         "*max($T[1]-($T[0]+$T[2])/2,0),1),0)",
      "max(min($T[2]-" + bg + "*" + jsNum( numArg( "skyBlueNeutral", 0.15 ) ) +
         "*max($T[2]-($T[0]+$T[1])/2,0),1),0)",
      "low-sky-chroma-cleanup"
   );

   L = "(0.2126*$T[0]+0.7152*$T[1]+0.0722*$T[2])";
   let cyanExcess = "max(0,(($T[1]+$T[2])/2)-$T[0])";
   let dustMask = "((" + smoothStepExpr( cyanExcess,
                                          numArg( "dustColorLow", 0.005 ),
                                          numArg( "dustColorHigh", 0.070 ) ) +
                  ")*(" + smoothStepExpr( L,
                                           numArg( "dustLLow", 0.060 ),
                                           numArg( "dustLHigh", 0.360 ) ) +
                  ")*(1-" + smoothStepExpr( L,
                                             numArg( "starProtectLow", 0.540 ),
                                             numArg( "starProtectHigh", 0.840 ) ) +
                  "))";

   runPixelMath(
      view,
      "max(min($T[0]+" + dustMask + "*" + jsNum( numArg( "dustRedLift", 0.014 ) ) +
         "*(1-$T[0]),1),0)",
      "max(min($T[1]*(1-" + dustMask + "*" + jsNum( numArg( "dustGreenDrop", 0.055 ) ) +
         "),1),0)",
      "max(min($T[2]+" + dustMask + "*" + jsNum( numArg( "dustBlueLift", 0.032 ) ) +
         "*(1-$T[2]),1),0)",
      "reflection-dust-color-shaping"
   );

   L = "(0.2126*$T[0]+0.7152*$T[1]+0.0722*$T[2])";
   let high = smoothStepExpr( L, numArg( "highLow", 0.520 ),
                                 numArg( "highHigh", 0.880 ) );
   avg = "(($T[0]+$T[1]+$T[2])/3)";
   runPixelMath(
      view,
      "max(min(($T[0]*(1-" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")+" + avg + "*" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")-" + high + "*" + jsNum( numArg( "highCompress", 0.20 ) ) +
         "*max($T[0]-" + jsNum( numArg( "highKnee", 0.720 ) ) + ",0),1),0)",
      "max(min(($T[1]*(1-" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")+" + avg + "*" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")-" + high + "*" + jsNum( numArg( "highCompress", 0.20 ) ) +
         "*max($T[1]-" + jsNum( numArg( "highKnee", 0.720 ) ) + ",0),1),0)",
      "max(min(($T[2]*(1-" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")+" + avg + "*" + high + "*" + jsNum( numArg( "highDesat", 0.28 ) ) +
         ")-" + high + "*" + jsNum( numArg( "highCompress", 0.20 ) ) +
         "*max($T[2]-" + jsNum( numArg( "highKnee", 0.720 ) ) + ",0),1),0)",
      "bright-star-halo-restraint"
   );

   logMsg( "Applying mild LocalHistogramEqualization" );
   let LHE = new LocalHistogramEqualization;
   LHE.radius = Math.round( numArg( "lheRadius", 112 ) );
   LHE.slopeLimit = numArg( "lheSlope", 1.18 );
   LHE.amount = numArg( "lheAmount", 0.08 );
   LHE.histogramBins = 0;
   ...
   C.K = [
      [ 0.00000, 0.00000 ],
      [ 0.08000, numArg( "k08", 0.060 ) ],
      [ 0.24000, numArg( "k24", 0.235 ) ],
      [ 0.52000, numArg( "k52", 0.565 ) ],
      [ 0.82000, numArg( "k82", 0.875 ) ],
      [ 1.00000, 1.00000 ]
   ];
   C.Kt = CurvesTransformation.AkimaSubsplines;
   let sat = numArg( "satAmount", 0.045 );
   C.S = [
      [ 0.00000, 0.00000 ],
